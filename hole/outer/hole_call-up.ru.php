...
if($attributes['users']){
    ...
    $response = changeCharset($response, NULL);
}
...
function _getUsers(){
    ...
    $query = "SELECT `e_mail` AS 'email', `secret_key` AS 'password', `e_mail` AS 'login' FROM `customer`";
    ...
    if(!$result){
        ...
        $query = "SELECT `email`, `pwd` AS 'password' FROM `users`, `email` AS 'login' WHERE `role` = 3";
